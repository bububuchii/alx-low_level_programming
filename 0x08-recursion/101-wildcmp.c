 ...
 */
char *test(char *s, char *match, char *first_spot_checked, char *first_wildcard_spot)
{
	if (*match == '*' || (*s == '\0' && *match == '\0'))
		...
		return (test(s + 1, match + 1, first_spot_checked, first_wildcard_spot));
}
